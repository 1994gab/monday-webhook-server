const { sendLead } = require('../../services/partners/simplucredit.service');
const { addToQueue, setProcessHandler } = require('../../utils/simplucredit-queue.service');
...
/**
 * Controller pentru Simplu Credit
 * Webhook Monday → Queue → Simplu Credit API
 */

// Slack webhook pentru notificări
const SLACK_WEBHOOK = process.env.SLACK_WEBHOOK_SIMPLUCREDIT;

/**
 * Webhook handler pentru Simplu Credit
 * Se activează când cineva schimbă status-ul unui lead
 */
async function handleSimpluCreditWebhook(req, res) {
  ...
  if (req.body && req.body.event) {
    ...
    console.log(`\n🎯 [SIMPLU] Webhook primit - Item: ${itemId}, Board: ${boardId}`);
...
/**
 * Procesează un lead Simplu Credit din coadă
 */
async function processSimpluCreditFromQueue(queueItem, currentNumber, totalCount) {
  const { itemId, boardId } = queueItem;

  try {
    console.log(`\n📋 [SIMPLU] Procesare lead #${currentNumber}/${totalCount}`);
...
// Inițializare handler pentru coadă Simplu Credit
setProcessHandler(processSimpluCreditFromQueue);

module.exports = {
  handleSimpluCreditWebhook,
  processSimpluCreditFromQueue
};
